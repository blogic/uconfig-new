...
export function dry_run(ctx) {
	let remote = model.uconfig.remote;
	if (remote)
		return remote.dry_run(ctx, model.uconfig.current_cfg);
...
export function commit(ctx) {
	let remote = model.uconfig.remote;
	if (remote)
		return remote.commit(ctx, model.uconfig.current_cfg);
