...
{%
	...
	function is_easymesh_controller(easymesh_mode) {
		return easymesh_mode in ['hybrid', 'controller'];
	}

	function is_easymesh_agent(easymesh_mode) {
		return easymesh_mode in ['hybrid', 'agent'];
	}
	...
	let easymesh_agent = false;
	if ('easymesh' in interface.services && state.services?.easymesh?.mode)
		easymesh_agent = is_easymesh_agent(state.services.easymesh.mode);
	...
	function generate_bridge_config(batman_result) {
		if (!interface.ports && length(interface.ssids) == 1 && !batman_result.batman_adv) {
			...
			include('interface/bridge-vlan.uc', {
				...
				batman_adv: batman_result.batman_adv, easymesh_agent
			});
		}
	}
