{%
	...
	function generate_basic_section(section, location, phy) {
		let output = [];
	...
	function generate_crypto_base(section, crypto, phy) {
		let output = [];

		uci_comment(output, '### generate crypto base settings');
		uci_set_number(output, `wireless.${section}.ieee80211w`, match_ieee80211w(phy));
		uci_set_string(output, `wireless.${section}.sae_pwe`, match_sae_pwe(phy));
	...
	function generate_rate_limit_rules(section) {
		...
		services.set_enabled('ratelimit', 'reload');
