...
function ipv6_leases(state) {
	// TODO: IPv6 lease tracking not yet implemented
	// The ubus dhcp.ipv6leases structure needs to be analysed to determine
	// the correct iteration pattern for extracting lease information
	return;
}
...
function interfaces(state) {
	...
	uci.foreach("network", "interface", function(d) {
		...
		if (devices && devices[role] && length(devices[role]["bridge-members"]))
			iface.bridge_members = devices[role]["bridge-members"];
