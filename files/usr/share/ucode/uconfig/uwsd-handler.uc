...
import * as usnitch from 'uwsd.usnitch';
...
function handle_usnitch(connection, id, params) {
	usnitch.handle((response) => send_response(connection, response), id, params);
}
...
let handlers = {
	...
	'usnitch': { handler: handle_usnitch, auth_required: true },
...
export function onConnect(connection, protocols) {
	...
	usnitch.connections_set(global.connections);
